...
function parseTermsContent(text: string) {
  if (!text) return null;
  ...
  return sections.map((section, idx) => {
    ...
    const content = section
      .replace(/\[TITLE\].*\n?/, "")
      .split("\n")
      .filter(line => line.trim() && !line.startsWith("[COPY]"))
      .map(line => line.replace(/^\[COPY\]\s*/, "").trim())
      .filter(line => line);
    ...
    return (
      <section key={idx}>
        ...
        {content.map((para, i) => (
          <p key={i} className="mb-2 text-[11px] text-[#333]/70 leading-[18px] tracking-[0.22px]">
            {para}
          </p>
        ))}
      </section>
...
export default function TermsPage() {
  const termsText = useQuery(api.platformSettings.get, { key: "termsText" });
  
  const hasCustomTerms = termsText && termsText.trim().length > 0;
